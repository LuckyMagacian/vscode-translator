...
import { SiliconFlowTranslator } from '../src/translators/SiliconFlowTranslator';
...
function testDefaultPromptPlaceholder() {
    ...
    const translator = new SiliconFlowTranslator(
        ...
        'test-model',
        'https://test.api',
        undefined,  // 不提供自定义提示词,使用默认
        '中文',
        '英文'
    );

    // 通过反射获取 systemPrompt (测试目的)
    const prompt = (translator as any).systemPrompt;

    // 验证占位符被替换
    assertEqual(
        prompt.includes('中文') && prompt.includes('英文'),
        true,
        '默认提示词中的占位符应被替换为实际语言'
    );
    ...
    // 验证不包含占位符本身
    assertEqual(
        !prompt.includes('{firstLanguage}') && !prompt.includes('{secondLanguage}'),
        true,
        '替换后的提示词不应包含占位符'
    );
