...
// 测试 1: 默认提示词占位符替换
function testDefaultPromptPlaceholder(): { pass: number; fail: number } {
...
// 测试 2: 自定义提示词占位符替换
function testCustomPromptPlaceholder(): { pass: number; fail: number } {
...
// 测试 3: 多次使用占位符
function testMultiplePlaceholders(): { pass: number; fail: number } {
...
// 测试 4: 无占位符的自定义提示词
function testNoPlaceholders(): { pass: number; fail: number } {
...
// 测试 5: 向后兼容性测试
function testBackwardCompatibility(): { pass: number; fail: number } {
...
// 测试 6: 非中英文语言配置
function testNonChineseEnglishLanguages(): { pass: number; fail: number } {
...
// 返回测试结果(供外部调用)
export { runTestSuite, replacePromptPlaceholders, DEFAULT_PROMPT_TEMPLATE };